...
fields_cleaned_list <- lapply(mget(sprintf("kernfields_%d", 2011:2017)), 
                 function(x) 
                   {
...
```{r}
#select fam fields that meet the criteria
fallowed_FAM <- FAM_binded %>% 
  filter(fallowed == 3) %>% 
  mutate(id = row_number() )#select fallowed fields (needs double checking!!)

fallowed_irrigated_FAM <- filter(id %in% unlist(st_intersects(fallowed_FAM, hist_irrigated)))

plot(st_geometry(fallowed_FAM))
plot(st_geometry(fallowed_irrigated_FAM))
```
