...
In this code chunk we read in soil data created previously in 3.0_SSURGOStorieInd_copy.rmd and rasterize it

```{r rasterize_soil_data}
...
soil_sf <- read_sf(here("R_input", "spatial", "SSURGOsoil", "kern_soil.gpkg")) %>% 
  ...
  #NOTE use of `interphr` as storie index
  dplyr::select(StorInd = interphr)
...
#rasterize
soil_ras = fasterize(soil_sf, ras, field = "StorInd")
