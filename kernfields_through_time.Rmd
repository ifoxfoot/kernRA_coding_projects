...
kernfield_join <- st_intersection(st_buffer(mosaic_drop_acre, dist = -1), kernfield_clean, left = T)

#pivot so each column is a year
kernfield_pivot <- kernfield_join %>% 
  separate(pmt_site_string, into = c("year", "pmt_site"), sep = "_") %>% 
  select(-c(FID, area_sq_ft)) %>% 
  pivot_wider(names_from = year, values_from = pmt_site) %>%
  st_sf() %>% 
  st_buffer(dist = 1)

#write to csv
write_csv(kernfield_pivot, "data/shapefiles_written/1997-2021_pivoted.csv")
```
