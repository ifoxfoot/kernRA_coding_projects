...
#create list of township range values
townships <- unique(all_fields$township_range)

#create list for resulting dfs
list_df <- list()

#create loop
for (i in townships) {
    df_name <- paste0("kernfields_", i)
    list_df[[df_name]] <- all_fields[all_fields$township_range == i,]
}

fields_processed <- lapply(list_df,
                           function(x) 
                             {
                             
                             dplyr::select(x, pmt_site_year) %>% 
                               summarise(geometry = st_combine(geometry)) %>%
                               st_cast("POLYGON")
                               
                             st_join(x, all_fields_single_col) %>% 
                               distinct() %>% 
                               st_make_valid() %>% 
...
#combine geometries
field_comb <- all_fields %>%
  summarise(geometry = st_combine(geometry)) %>%
  st_cast()

#add ID data back to geometry, get rid of duplicates
field_join <- st_join(field_comb, all_fields_single_col) %>% 
  distinct() %>% 
  st_make_valid()
