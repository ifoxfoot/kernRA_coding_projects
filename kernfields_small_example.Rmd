...
fallowed_sample_pivot <- fallowed_sample_pivot %>%
  mutate(!!paste0("fallowed_", i) 
         := case_when(!!as.name(i) == "NULL" ~ "fallowed_null",
                      str_detect(!!as.name(i), "-66000-") & 
                        str_detect(!!as.name(i), ", ", negate = T) ~ "fallowed_pmt",
                            T ~ "not_fallowed"))
}

#create another row number 
fallowed_sample_pivot <- fallowed_sample_pivot %>% 
...
sample_long_mutate = sample_long %>%
  ...
  mutate(p_var = paste0("p", substr(year, 10, 14))) %>% 
...
#library(zoo)
# 
# #select only active pmt_site columns
# just_pmt <- years_fallowed %>% 
#   mutate(`2016` = NA_character_) %>% 
#   select(`2016`,`2017`, `2018`, `2019`) %>% 
#   mutate_all(., function(x) case_when(
#     str_detect(as.character(x), "-66000-") & 
#       str_detect(as.character(x), ", ", negate = T) ~ NA,  
#     T ~ as.character(x)))
# 
# 
# #needs to be casewhen fallowed, is NA
# 
# #replace null values with proper NAs
# just_pmt[just_pmt == "NULL"] <- NA_character_
#   
# #spread to wide format
# just_pmt_spread <- just_pmt %>% 
#   unnest_wider(c(`2017`, `2018`, `2019`), names_sep = "_")
# 
# #fill NAs to the right
# d <- t(na.locf(t(just_pmt_spread))) %>% 
#   as.data.frame()
```
