...
sample_pivot_sep <- fallowed_sample_pivot_2019 %>% 
...
# Creating table with years fallowed and last crop

```{r}
#identifying fallowed fields function (in progress)
years <- c(2017, 2018, 2019)

#create dataset for for for loop to loop over
fallowed_sample_pivot <- sample_pivot

#for loop that creates a new column per each year, indicating when it was fallowed
for (i in years) {
...
```{r}
#find how long 2019 fallowed sites have been fallowed
years_fallowed <- fallowed_sample_pivot_2019 %>% 
  mutate(yrs_fallowed = case_when(
    fallowed_2019 != "fallowed" ~ "0",
    fallowed_2019 == "fallowed" & fallowed_2018 != "fallowed" ~ "1",
    fallowed_2019 == "fallowed" & fallowed_2018 == "fallowed" & fallowed_2017 != "fallowed" ~ "2",
    fallowed_2017 == "fallowed" & fallowed_2018 == "fallowed" & fallowed_2019 == "fallowed" ~ "3",
    T ~ "missing"))

#find out what the pmt_site(s) were before fallowed
last_crop <- years_fallowed %>%
  mutate(last_crop = case_when(yrs_fallowed == "0" ~ as.character(`2019`),
                               yrs_fallowed == "1" ~ as.character(`2018`),
                               yrs_fallowed == "2" ~ as.character(`2017`),
                               TRUE ~ "always_fallowed"))

plot(last_crop["yrs_fallowed"])
```
...
long_long = 
df_long %>%
mutate(p_var = paste("p", substr(week_num, 5, 5), sep="")) %>% 
  #created p-columns that correspond to respective weeks arrange(id, week_num) %>%
group_by(id) %>%
mutate(active_var = ifelse(week_orders != 0, "active", 
                  ifelse(first_active < as.numeric(substr(week_num, 5,5)),
                         "lapsed", NA))) %>% 
#%>% #created a column that would return either "active", "lapsed" or NA depending on user activity
     mutate(lapsed_num =  sequence(rle(active_var)[["lengths"]])) #created a column that would count the number of occurences of "lapsed" for a given id; it would start counting from 1 if after "active" appeared 
            final = ifelse(active_var == "active", active_var, 
                           ifelse(active_var == "lapsed", paste(active_var, lapsed_num, sep=""), NA))) 
     #%>% #finally, the column takes "active" status or coalesces "lapsed" with the sequence number
...
sample_long = melt(fallowed_sample_no_geo, id.vars = c("2017", "2018", "2019", "area_sq_ft", "fallow_count_id")) #transformed my wide data to the long format
colnames(sample_long) = c("2017", "2018", "2019", "area_sq_ft", "fallow_count_id", "year", "status")


sample_long_mutate = sample_long %>%
  mutate(p_var = paste("p", substr(year, 10, 14), sep = "")) %>% 
  #created p-columns that correspond to respective weeks arrange(id, week_num)
...
df_p = dcast(sample_long_mutate[, c(1:7, 8:9)], fallow_count_id  ~ p_var,  value.var = "lapsed_num")

years_fallowed = merge(fallowed_sample_pivot, df_p, id = fallow_count_id)
```
